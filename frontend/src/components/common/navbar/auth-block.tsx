      ...
      {user && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size={"default-icon"}>
              <Image
                src={user.profileImg}
                alt="user avatar"
                width={35}
                height={35}
                className="rounded-full object-cover size-[35px]"
              />
          ...
          <DropdownMenuContent>
            <DropdownMenuLabel>{m.raw_front_goat_scold()}</DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem asChild>
              <Link href="/profile">{m.bad_caring_wallaby_pull()}</Link>
            </DropdownMenuItem>
