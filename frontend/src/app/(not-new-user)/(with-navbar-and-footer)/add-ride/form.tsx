      ...
      >
        <div className="grid grid-cols-2 gap-5">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{m.bald_wild_rabbit_fall()}</FormLabel>
                <FormControl>
                  <Autocomplete
                ...
                <FormMessage errorMessage={m.chunky_weird_peacock_pick()} />
              </FormItem>
            )}
          />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{m.teary_zesty_squirrel_bend()}</FormLabel>
                <FormControl>
                  <Autocomplete
                ...
                <FormDescription>{m.giant_born_jaguar_vent()}</FormDescription>
                <FormMessage errorMessage={m.tiny_trite_eel_bloom()} />
              </FormItem>
            )}
          />
        </div>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{m.shy_mellow_florian_strive()}</FormLabel>
              <FormControl>
                <Select
                  value={field.value}
                  onValueChange={(v) => {
                ...
                >
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder={m.honest_ideal_jan_bump()} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      {cars.map(({ id, name }) => (
              ...
              <FormMessage errorMessage={m.just_clear_reindeer_gleam()} />
            </FormItem>
          )}
        />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{m.mushy_home_cougar_embrace()}</FormLabel>
              <FormControl>
                <Select
                  key={selectedCar?.id}
              ...
              <FormMessage errorMessage={m.trite_novel_sheep_intend()} />
            </FormItem>
          )}
        />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>{m.many_strong_moose_enchant()}</FormDescription>
              <FormMessage errorMessage={m.honest_chunky_butterfly_delight()} />
            </FormItem>
          )}
        />
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                ...
                {((bestPriceValue || 0) / (selectedCar?.capacity || 1)).toFixed(
                  1
                )}{" "}
                {m.flaky_wide_samuel_drum()}
              </FormDescription>
              <FormMessage errorMessage={m.next_light_hyena_savor()} />
            </FormItem>
          )}
        />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{m.super_agent_albatross_animate()}</FormLabel>
              <FormControl>
                <ReactSelect
                  isMulti
                  name="rules"
                  ...
                  options={rules.map((r) => ({
                    value: r.id,
                    label: ruleToLabel[r.description][languageTag()],
                  }))}
